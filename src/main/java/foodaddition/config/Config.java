...
public class Config {
    ...
    public static boolean brownSugarEnabled, grilledFoodEnabled, sheepFoodEnabled, squidFoodEnabled, horseFoodEnabled, wolfFoodEnabled;
    ...
    public static void init(File file) {
        config = new Configuration(file, "1.3");
        config.load();
        loadConfig();
    }
    ...
    private static void loadConfig() {
        ...
        grilledFoodEnabled = config.get(itemsCat, "GrilledFood", false).getBoolean(false);
        ...
        config.addCustomCategoryComment(grassDrops, "Here you can tweak new drops added when breaking grass");
        wheatDropFromGrass = config.get(grassDrops, "Wheat Drop From Grass", true).getBoolean(true);
        wheatDropWeight = config.get(grassDrops, "Wheat Drop chance", 2, "Wheat drop chance from grass. 1 means 1/80 or 1.25% ; 10 means 1/8 or 12.5%", 1, 10).getInt(2);
